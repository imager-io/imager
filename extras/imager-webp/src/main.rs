...
pub fn load_image(data: Vec<u8>) -> WebPPicture {
    let mut picture: WebPPicture = unsafe {
        std::mem::zeroed()
    };
    unsafe {
        ffi::cbits::utils::webp_picture_from(
            data.as_ptr(),
...
pub fn open_image<P: AsRef<Path>>(path: P) -> WebPPicture {
    let data = std::fs::read(path).expect("open image failed");
    load_image(data)
}

pub fn main() {
    let input_path = PathBuf::from("2yV-pyOxnPw300.jpeg");
    assert!(input_path.exists());
    let config = encoder_config(75.0);
    let picture = open_image(&input_path);
}